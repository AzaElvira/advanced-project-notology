...
if __name__ == "__main__":
    ...
    # Записываем данные о пользователях, полученных через vk_search,
    # в базу данных в таблицу User. На вход подаётся список, см. fixtures.users
    vkinder.add_user_data(data=users)

    # Получаем данные о пользователе по его порядковому номеру из базы User,
    # порядковые номера начинаются с 1, далее 2, 3 и т.д.
    ...
    # Записываем url фото для пользователя в базу данных в таблицу Photo.
    # Url получаем через vk_search. На вход подаётся список, см. fixtures.urls
